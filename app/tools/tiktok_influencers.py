...
async def search_tiktok_influencers(query: str, limit: int = 10) -> Dict[str, Any]:
    ...
    """
    try:
        logger.info(f"TikTok Query: {query}, Limit: {limit}")
        result = await query_vector_store(query, "tiktok", limit)
        logger.info(f"TikTok Results: {len(result)} influencers found")
        ...
        logger.error(f"Error in search_tiktok_influencers: {str(e)}")
        return {
            "error": True,
            "message": str(e) or "Unknown error occurred while searching TikTok influencers"
        }