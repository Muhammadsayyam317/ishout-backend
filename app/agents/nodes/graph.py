...
async def node_requirements(state: ConversationState):
    ...
    logging.info(f"[node_requirements] msg={msg}")
...
async def node_send(state: ConversationState):
    ...
    try:
        ...
        logging.error(f"Error resetting user state for sender_id: {state['sender_id']}")
    return state
...
def missing_fields(state: ConversationState):
    return [
        f
        for f in ["platform", "country", "number_of_influencers", "category"]
        if not state.get(f)
    ]
